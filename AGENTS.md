...
- Tests never hit live sources and never mirror data contents. pytest runs with `--block-network`; HTTP goes through cassettes in `tests/cassettes/`. Re-record with `just record` only when an adapter or a source changes.
