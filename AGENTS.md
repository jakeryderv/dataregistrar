...
- `src/dataregistrar/`: the package. `model` has no I/O; dependencies point downward only (`cli → search → registry → adapters → model`).
- `tests/unit/` for pure logic, `tests/contract/` for the adapter suite, `tests/cassettes/` for recorded HTTP.

## Constraints

- Tests never hit live sources and never mirror data contents.
